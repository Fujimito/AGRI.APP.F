...
function ProgressMapTab(p) {
  ...
  /*#__PURE__*/React.createElement("div", {
  ...
  }, showLabels ? "🏷 札あり" : "🏷 札なし"), /*#__PURE__*/React.createElement("button", {
    ...
    style: fullMap ? {
      ...
      // ── 全画面の操作は下の帯に集める ──
      //
      // v8.89 までは右上に縦積みしていたが、上端は当てにできない。
      // 端末ごとに env(safe-area-inset-top) が違ううえ、実機(iPhone)では
      // 覆い(mapWrapFull)が見出しとタブバーを隠しきれず、上の2つが
      // その裏に回って押せなくなっていた(手元のエミュレータでは
      // safe-area が 0 のため再現できず、実機の画面で分かった)。
      //
      // 下の帯なら、ノッチも覆いの効き方も関係ない。作図の全画面で
      // 既に使っている形(drawBarFull)と同じで、親指も届く。
      // 入りきらないときは横に流す。畳んで2段にすると地図が削られる。
      style: S.mapFullBar,
      className: "no-print"
    }, /*#__PURE__*/React.createElement("button", {
      onClick: () => setFullMap(false),
      style: S.mapFullBarBtn
    }, "✕ 全画面"), /*#__PURE__*/React.createElement("button", {
      onClick: () => apiRef.current && apiRef.current.locate && apiRef.current.locate(),
      style: S.mapFullBarBtn,
      title: "今いる場所へ地図を寄せる"
      ...
      style: S.mapFullBarBtn,
      title: "その日の作業に入っている圃場が全部入るまで寄せ直す"
    }, "⊙ 今日の圃場"), /*#__PURE__*/React.createElement("button", {
      // 重くなるのは全画面のときが一番はっきり出る。ここで消せないと、
      // 一度全画面をやめて上のツールバーまで戻ることになる
      onClick: () => setShowLabels(!showLabels),
      style: {
        ...S.mapFullBarBtn,
        ...(showLabels ? {} : S.mapFullBarBtnOff)
      },
      title: "圃場名と面積の札を出す・消す"
    }, showLabels ? "🏷 札あり" : "🏷 札なし"))), !fullMap && /*#__PURE__*/React.createElement("button", {
    onClick: () => setFullMap(true),
    style: {
      ...S.smallSecondary,
      marginTop: 8
...
const S = {
  ...
  // 進捗地図を全画面にしたときの操作帯。
  //
  // position:fixed をやめた(v8.92)。iPhone15 で、上端に置いた v8.89 では
  // 覆い(mapWrapFull)が見出しを隠しきれず上の2つが裏に回り、下端に移した
  // v8.90 では帯そのものが出なくなった。どちらも fixed の基準が画面に
  // なっていないと考えると説明が付く。手元では再現できない。
  //
  // この帯は覆いの子で、覆いは display:flex / column。ならば「一番下の子」
  // として置けばよく、fixed も z-index も要らない。覆いがどこにどう
  // 置かれても、必ず地図の下端に出る。
  // 下の余白は覆い側(mapWrapFull)が safe-area ぶん確保しているので、
  // ここでは足さない(足すと二重になる)。
  //
  // 幅が足りなければ横に流す。折り返して2段にすると地図が削られる。
  mapFullBar: {
    flexShrink: 0,
    display: "flex",
    alignItems: "center",
    // "safe center" にする。ただの center だと、幅が足りないときに左側へ
    // はみ出し、スクロールでも届かない位置に切れる(375pxで実際に起きた)。
    // safe を付けると、収まらないときだけ先頭寄せに落ちる。
  ...
  // 文字と余白は、4つが360px幅に1行で収まるようブラウザで実測して詰めた。
  // 360pxは今どき最も狭い部類のスマホ。ここが収まれば375も768も1280も収まる。
  // 絵文字の幅は環境で変わるので、机上の計算ではなく実際に測って決めること。
  mapFullBarBtn: {
    // はみ出させるくらいなら縮める。絵文字の幅は環境で変わり、iOSはこの実測
    // (Chromium)より広い。余裕は9pxしかないので、足りない端末では最後の1つが
    // 画面外へ出る。縮めば文字は詰まるが、4つとも画面内に残って押せる。
    flexShrink: 1,
    minWidth: 0,
    overflow: "hidden",
    textOverflow: "ellipsis",

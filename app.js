...
// 全画面のときに右上へ縦に積むボタンの位置。i は上からの番号(0が一番上)。
// ノッチのある端末でも被らないよう、上端は safe-area から測る。
const mapFullBtnAt = i => ({
  ...S.mapFullBtn,
  top: "calc(" + (10 + i * 46) + "px + env(safe-area-inset-top))"
});
...
function ProgressMapTab(p) {
  ...
  /*#__PURE__*/React.createElement("div", {
    ...
    }), fullMap && /*#__PURE__*/React.createElement(React.Fragment, null,
    // 全画面にすると上のツールバーごと隠れ、押したい場面なのに手が届かない。
    // 右上に上から順に積む。位置は mapFullBtnAt が番号から計算する。
    /*#__PURE__*/React.createElement("button", {
      onClick: () => setFullMap(false),
      style: mapFullBtnAt(0)
    }, "✕ 全画面をやめる"), /*#__PURE__*/React.createElement("button", {
      onClick: () => apiRef.current && apiRef.current.locate && apiRef.current.locate(),
      style: mapFullBtnAt(1),
      title: "今いる場所へ地図を寄せる"
    }, "📍 現在地"), /*#__PURE__*/React.createElement("button", {
      onClick: fitToTargets,
      style: mapFullBtnAt(2),
      title: "その日の作業に入っている圃場が全部入るまで寄せ直す"
    }, "⊙ 今日の圃場へ"), /*#__PURE__*/React.createElement("button", {
...
const S = {
  ...
  // 全画面のときに右上へ縦に積む操作ボタン。位置は mapFullBtnAt が番号から
  // 計算するので、増やしてもここに定義を足さなくてよい。
  mapFullBtn: {
    position: "fixed",
    right: 12,
    zIndex: 910,

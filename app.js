...
//
// 出さない場合(null)は「現在地が出ている」ときだけ。出ているのに案内を
// 出しても邪魔なだけで、出ていないときに何も出ないのが今までの問題だった。
// 拒否のあとはブラウザが二度と確認を出さないので、押しても許可は出せない。
// そこは端末の設定へ案内する(guide: true)。
//
// state はブラウザの Permissions API の値。iOS Safari は 16 から返すが、
// 返さない環境もあるので "unknown" を許す。分からないときは案内を出す側に
// 倒す。押せば確認が出るなら出したいし、出ないなら状況が分かる。
const geoHintFor = (state, hasFix) => {
  if (hasFix) return null;
  if (state === "unsupported") return {
    text: "この端末は位置情報に対応していません",
    guide: true
  };
...
// 見て回りたい。ところが今日の作業に入れていない圃場は、前の日に散布し
// 終えていても対象外(黄)になり、赤(今日やる)との区別しか付かない。
// 済んだ場所へまた向かうことになる。
//
// そこで選んでいる日だけでなく、さかのぼって数日ぶんの実績を見る。
//
//   今日の作業にあり、全部済     → done      (緑)
//   今日の作業にあり、1件でも未  → planned   (赤)   ※v8.83 の規則
//   今日の作業に無く、前の日に済 → donePrev  (青)
//   どれでもない                 → 出さない  (呼び側で黄)
//
// 「今日の作業に無く、前の日も済んでいない」は出さない(黄のまま)。
// それは予定に入れていない圃場で、引き継ぎボタンを押していないだけ。
// ここで赤にすると、今日回る予定でない圃場まで赤くなって数が合わなくなる。
//
// entries は作業1件ぶん。同じ作業がサーバーと手元の両方から来るので、
// 呼び側が作業IDで重ねてから渡す(重複を数えると件数が倍になる)。
const foldProgress = (entries, day) => {
  const m = new Map();
  (entries || []).forEach(e => {
    const key = String(e.fieldKey);
    let cur = m.get(key);
    if (!cur) {
      cur = {
        status: "none",
    ...
    const today = String(e.workDate || "") === String(day);
    if (today) {
      cur.total++;
    ...
    // 前の日ぶん。済んだものだけ拾う。前の日の未実施は、今日の予定に
...
// 日付を n 日ぶんさかのぼった "yyyy-MM-dd" を返す。
// 進捗の取得範囲の始まりに使う。
const daysBefore = (ymd, n) => {
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(ymd || ""));
  if (!m) return String(ymd || "");
  // 月をまたぐ・年をまたぐ・うるう年を自分で数えない。Date に任せる。
  // 時刻は正午にする。深夜0時だと夏時間や端末の時計のずれで前日に転ぶ
...
// ── 引き継ぐ圃場を選ぶ ──
//
// 直近の日にやり残した圃場を、今日のリストへ入れるための一覧。
//
// 選ぶ条件は3つとも満たすもの。
//   (1) 期間内(選んだ日より前)に、まだ済んでいない作業がある
//   (2) 期間内に、済んだ作業が1件も無い
//   (3) 今日のリストにまだ入っていない
//
// (2) が要る。引き継いだあとも前日の作業は残す方針なので、前日の未実施は
// ずっと未実施のまま残る。(2) が無いと、今日済ませた圃場が翌日もまた
// 引き継ぎ候補に出続ける。
//
// 割り切り: 同じ日に同じ圃場を2件入れて(午前と午後で分けた等)片方だけ
// 済んだ場合、(2) に当たるので引き継がない。地図はその日を未実施(赤)で
// 出すので食い違うが、「一度は撒いた圃場」を毎日蒸し返すほうが困る。
// 残りは手で足せる。
const carryOverFieldIds = (works, day, days) => {
  const from = daysBefore(day, (days || PROGRESS_CARRY_DAYS) - 1);
  const todays = new Set();
  const done = new Set();
  const left = [];
  const seen = new Set();
  (works || []).forEach(w => {
    const d = String(w.workDate || "");
    const k = String(w.fieldId);
    if (d === String(day)) {
      todays.add(k);
      return;
    }
    if (!d || d < from || d > String(day)) return;
    if (w.reported) done.add(k);
  });
  (works || []).forEach(w => {
    const d = String(w.workDate || "");
    const k = String(w.fieldId);
    if (!d || d >= String(day) || d < from) return;
    if (w.reported || todays.has(k) || done.has(k) || seen.has(k)) return;
    seen.add(k);
    left.push(w.fieldId);
  });
  return left;
...
function App() {
  ...
  // 進捗を送るのは送信ボタンを押したときだけにする。
  // (v9.41: 送信は「☁ 作業予定を共有」と「☁ 実績を送信」の2つに分けた)
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 送信ボタン(予定共有/実績送信)を押したときだけ。圃場マスタと薬剤マスタは
  ...
  const carryOverWorks = () => {
    ...
    if (ids.length === 0) {
      flash("引き継ぐ圃場はありません(直近" + PROGRESS_CARRY_DAYS + "日のやり残しはすべて済んでいます)");
      return;
    }
    const names = ids.map(id => {
      const f = fields.find(x => x.id === id);
      return f ? f.name : "(名前なし)";
    });
  ...
  const removeServerWorks = ids => {
    ...
    addTomb("works", list);
    flash(list.length + "件を外しました。「☁ 作業予定を共有」を押すとサーバーからも消えます");
  };
  const removeWorks = ids => {
    const set = new Set(ids);
    if (set.size === 0) return;
    // 墓標を積んで送るところまでやる。v8.71 まではここだけ抜けていて、
    // まとめて外した圃場がシートに残り、進捗地図で未実施(赤)のままになっていた。
    // 1件ずつ外す removeWork には入っていたので、「外す」だと消えて
    // 「🗑 選択して削除」だと消えない、という差になっていた。
    addTomb("works", [...set]);
    setWorksSave(works.filter(w => !set.has(w.id)));
    flash(set.size + "件をこの日のリストから外しました。「☁ 作業予定を共有」を押すと他の端末にも反映されます");
  };
    ...
    updateWorks(cur => cur.map(x => {
      ...
      if (x.reported) {
        return {
          ...
          reported: false,
          sprayedL: 0,
          // 薬液は実績を入れたときに焼き付けている(v9.33)ので、
          // 取り消すならそれも戻す。残すと「撒いていないのに薬剤がある」
          // 記録になり、台帳の行も調合済のまま薬剤だけ残る
          chems: [],
          totalL: 0,
          waterMl: 0,
          flights: [],
  ...
  const bulkReportFromRate = ratePer10a => {
    ...
    const next = cur.map(w => {
      ...
      updated++;
      const sprayed = plannedLFromArea(area, rate);
      return {
        ...w,
        // 実績が入った時点でその日の薬液を焼き付ける(v9.33)。
        // 個別入力(submitReport)と同じ扱いにしないと、一括で入れた圃場だけ
        // 薬剤の無い記録になる
        ...stampDayChems(w, sprayed),
        sprayedL: sprayed,
        reportAreaA: area,
    ...
    flash(updated + "圃場に実績を入れました" + (noArea > 0 ? "(面積未入力 " + noArea + "件は対象外)" : "") + "。送信は下の「☁ 実績を送信」から");
  };
  ...
  // 薬剤が決まるので、その時点で調合済の行が共有され、あとで薬剤を変えたり
  // その日をやめたりすると食い違いが残っていた。その日は全圃場で同じ薬液を
  // 使う運用なので、薬剤は日に1セット入力するだけにして、圃場ごとの適用を
  // やめた。薬量は実際に撒いた量から出すので、予定を基準にするより実態に合う。
  const stampDayChems = (w, sprayedL) => {
    const list = validDayChems;
    if (!list.length) return {};
    const per = parseFloat(sprayedL) || 0;
    if (!(per > 0)) return {};
    const perMl = per * 1000;
    const scaled = list.map(c => scaleChem(c, perMl));
    const chemMlSum = scaled.reduce((sum, c) => sum + c.ml, 0);
    return {
      chems: scaled,
      totalL: per,
  ...
  const submitReport = (id, rep) => {
    ...
    setWorksSave(next);
    // 使った薬剤は薬剤マスタと「↩前回と同じ薬液」に残す。
    // v9.32 までは圃場への適用のときにやっていた
    if (validDayChems.length) {
      upsertChemMaster(validDayChems.map(c => ({
        name: c.name || "(無名)",
        form: c.form,
        use: c.use || "other"
      })));
    ...
    flash(validDayChems.length
      ? "実績を保存しました(この日の薬剤を記録しました)。作業終了後に「☁ 実績を送信」を押してください"
      : "実績を保存しました。作業終了後に「☁ 実績を送信」を押してください");
  ...
  // ── 作業の送信の本体(v9.41で予定共有/実績送信に分けた共通部) ──
  // 送る対象(pend)と削除の墓標(tombs)を呼び出し側で決め、ここは
  // 「渡されたものを送って印を立て直す」だけを担う。
  // 圏外や未設定なら黙って諦める。未送信のまま残るので、あとから手で送れる。
  //
  // syncing の共有について: 送信ボタン・見出しのバッジ・設定タブの送り直しなど、
  // この送信はあちこちから呼ばれる。同時に走ると tankmix:works の読み直し→
  // 書き戻しが競合し、片方の印が後勝ちで消える恐れがあるため、他の送信系
  // (cloudSave/cloudLoad/syncShared)と同じ syncing 状態を共有して連打・重複起動を
  // UI側でも止める。分けなかった理由: これらは全部 localStorage の同じコレクション
  // (tankmix:works・tankmix:fields 等)を丸ごと読み書きするので、状態を分けても
  // 「こちらは止まっているが syncShared は動いている」間に works を書き換えられると
  // 結局衝突する。1つの状態で足並みを揃えるほうが単純で安全と判断した。加えて
  // この送信は呼ばれる頻度が桁違いに高いので、state の反映(非同期)を待たずに
  // 弾ける ref(pushingRef)も別に持たせている(cloudSave 等は手動操作でしか
  // 呼ばれないため無くても実害が薄く、そこまでは揃えていない)。
  const pushWorks_ = async (pend, tombs, opt) => {
    const quiet = opt && opt.quiet;
    const doneMsg = (opt && opt.doneMsg) || "送信しました";
    const failMsg = (opt && opt.failMsg) || "送信に失敗しました";
    const emptyMsg = (opt && opt.emptyMsg) || "送っていないものはありません";
    if (!syncReady()) {
    ...
    try {
      ...
      if (!r.ok) {
        if (!quiet && r.error !== "auth") flash(failMsg + (r.error ? "(" + r.error + ")" : ""));
        return false;
      }
      ...
      // 変えない)。警告はセッション中1回だけに絞る(oldGasLedgerWarnedRef)
      ...
      save(TOMB_KEY, t);
      if (!quiet) flash(doneMsg + "(" + items.length + "件)");
      return true;
    } finally {
  ...
  //   ② cur.filter(isPending) … 台帳の印がまだ立っていないもの
  // ①だけでは足りない。他端末の更新を pull で取り込むと、pushedAt は updatedAt に
  // 追いつく(itemToWork)一方、synced 等は「この端末の事情」として残るため、①からは
  // 外れるのに②には残り続ける。progressTargets がこの合流を担う。
  //
  // v9.41: 送信を「予定共有」と「実績送信」の2つに分けた。同じ pushWorks 経路だが、
  // 未実施(!reported=予定・調合済)と実施済(reported)で送るボタンを分ける。
  // GAS側はID単位のupsertなので、片方だけ送っても他方の行は消えない。
  //
  // 予定共有: 未実施の作業＋削除の墓標を送る。台帳(防除記録)には載らない
  // (GASが status!=="done" の行を台帳へ書かないため)。
  const pushPlan = async opt => {
    const cur = load("tankmix:works", []);
    const pend = progressTargets(cur).filter(w => !w.reported);
    const tombs = loadTombs().works;
    return pushWorks_(pend, tombs, {
      quiet: opt && opt.quiet,
      doneMsg: "作業予定を共有しました",
      failMsg: "作業予定の共有に失敗しました",
      emptyMsg: "共有していない予定はありません"
    });
  };
  // 実績送信: 実施済(reported)の作業を送る。墓標は予定側で送るのでここでは送らない。
  // これを受けたGASが「作業」シートと「防除記録」(台帳)の両方に書く。
  const pushResults = async opt => {
    const cur = load("tankmix:works", []);
    const pend = progressTargets(cur).filter(w => w.reported);
    return pushWorks_(pend, [], {
      quiet: opt && opt.quiet,
      doneMsg: "実績を送信しました",
      failMsg: "実績の送信に失敗しました",
      emptyMsg: "送っていない実績はありません"
    });
  };
  ...
  const fetchProgress = async (from, to) => {
    // 共有オフと未設定は直し方が違う。まとめて「設定」で返すと、
    // URLもチームも入っているのに設定タブへ案内されてしまう
    if (!shareOn) return {
      error: "共有オフ"
    };
    if (!syncReady()) return {
  ...
  // v9.41: 見出しの送信バッジは「実績が未送信のときだけ」出す(本人の指定)。
  // 予定の共有は「本日の作業圃場登録」の中のボタンで行うので、ここには出さない。
  const resultPendingCount = works.filter(w => w.workDate === workDate && w.reported && isPending(w)).length;

  // v9.38: 電波が戻ったときの自動送信もやめた。進捗を送るのは
  // 送信ボタン(予定共有/実績送信)を押したときだけ。未送信は件数で見えているので、
  // いつ送るかは人が決める。
  ...
  }, teamCode.trim() ? "👥 " + teamCode.trim() : "👥 チーム未設定"), resultPendingCount > 0 && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setTab("work");
      pushResults();
    },
    style: S.headerBadge
  }, syncing ? "送信中…" : "☁ 実績を送信(未送信 " + resultPendingCount + "件)"))), saveFail && /*#__PURE__*/React.createElement("div", {
    style: {
      ...S.warnBand,
      background: "#FDE2E2",
      borderColor: "#E58B8B"
...
function WorkTab(p) {
  ...
  // ただし送信ボタン(pushPlan/pushResults)自体は日をまたいでたまっている分をまとめて送る
  const pendingWorks = dayList.filter(w => !w.synced || w.reported && !w.reportSynced);
  const pending = pendingWorks.length;
  // v9.41: 送信を「予定共有」と「実績送信」に分けた。未実施(!reported=予定・調合済)は
  // 予定共有ボタンの、実施済(reported)は実績送信ボタンの件数として別々に数える。
  const planPending = pendingWorks.filter(w => !w.reported).length;
  const resultPending = pendingWorks.filter(w => w.reported).length;
...
/*#__PURE__*/React.createElement(WorkProgress, {
  ...
  collapsibleHead("🌾 本日の作業圃場登録", prepOpen, () => setPrepOpen(!prepOpen)), prepOpen && /*#__PURE__*/React.createElement(React.Fragment, null,
  // v9.41: 予定共有ボタン。ここで登録した「本日の作業圃場」を他の端末へ配る。
  // 実績送信(散布後)とは別のボタンにして、作業前後で押すものを分ける。
  dayList.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => p.pushPlan(),
    disabled: p.syncing || planPending === 0 || !p.gasUrl,
    style: {
      ...S.smallPrimary,
      width: "100%",
      padding: "13px 0",
      background: "#2F6FA8",
      borderColor: "#24567F",
      opacity: p.syncing || planPending === 0 || !p.gasUrl ? 0.4 : 1
    }
  }, p.syncing ? "送信中…" : !p.gasUrl ? "☁ 送信先が未設定です" : planPending === 0 ? "☁ 共有していない予定はありません" : "☁ 作業予定を共有(未共有 " + planPending + "件)"), /*#__PURE__*/React.createElement("p", {
    style: {
      ...S.note,
      marginTop: 4
    }
  }, "この日に登録した圃場(予定)を他の端末へ共有します。散布後の実績は、下の地図/一覧の「☁ 実績を送信」から送ります。")), dayList.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: S.rateBox
  }, /*#__PURE__*/React.createElement("div", {
    style: S.smallLabel
    ...
    onClick: () => {
      if (confirm("この日の薬剤欄をすべて消しますか？\n(実績を入れて記録済みの薬剤は消えません)")) p.clearDayChems();
    },
    style: S.linkBtn
  ...
  }, "この日に使う薬剤(薬剤名と希釈倍率)を入力します。実績を入れた圃場に、その日の薬液がそのまま記録されます。") : /*#__PURE__*/React.createElement("div", {
  ...
  }, p.dayChems.length === 0 ? "＋ この日の薬剤を入力" : "✎ この日の薬剤を編集")), dayChemsOpen && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 6
    }
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "この日に使う薬剤"), p.dayChems.length === 0 && /*#__PURE__*/React.createElement("p", {
    style: S.empty
  ...
  }, "↩ 直近", PROGRESS_CARRY_DAYS, "日のやり残し ", p.carryOverCount, "圃場を引き継ぐ"), activeZone && results.length > 0 && /*#__PURE__*/React.createElement("button", {
    ...
    style: S.empty
  }, "まだ圃場が登録されていません。", /*#__PURE__*/React.createElement("br", null), "「🗺圃場登録・圃場一覧」タブの地図で囲んで登録してください。") : /*#__PURE__*/React.createElement(React.Fragment, null, p.fields.length > 4 && /*#__PURE__*/React.createElement("input", {
    value: query,
...
/*#__PURE__*/React.createElement("section", {
    ...
    style: S.cardLabel
  }, "作業終了後に実績を送信(未送信 ", resultPending, "件)"), !p.gasUrl && /*#__PURE__*/React.createElement("p", {
    style: {
      ...S.memoLine,
      marginBottom: 10
    }
    ...
    disabled: p.syncing || resultPending === 0 || !p.gasUrl,
    style: {
      ...S.bigSendBtn,
      opacity: p.syncing || resultPending === 0 || !p.gasUrl ? 0.45 : 1
    }
  }, p.syncing ? "送信中…" : !p.gasUrl ? "☁ 送信先が未設定です" : resultPending === 0 ? "☁ 送信する実績はありません" : "☁ 実績を送信(未送信 " + resultPending + "件)"), /*#__PURE__*/React.createElement("p", {
    style: S.note
  }, "散布し終えた圃場の実績を送ります。これを受けたスプレッドシート側が「作業」と「防除記録」(台帳)の両方に書きます。予定の共有は「🌾 本日の作業圃場登録」の中のボタンから。電波のある場所で押してください。送信済みは二重登録されません。")), /*#__PURE__*/React.createElement("section", {
    style: S.card,
    id: "print-area"
  }, /*#__PURE__*/React.createElement("div", {
    style: {
...
function SettingsTab(p) {
  ...
  }, "📋 作業予定を送り直す"), /*#__PURE__*/React.createElement("button", {
    onClick: () => p.pushResults(),
    disabled: p.syncing,
    style: {
      ...S.secondaryBtn,
  ...
  }, "🚦 実績を送り直す"), /*#__PURE__*/React.createElement("p", {
    style: S.note
  }, "作業タブの送信ボタンを押す運用です(v9.38以降、自動では送りません)。ここは圏外だったときの送り直し用です。「作業予定」は登録した圃場(予定)を、「実績」は散布し終えた圃場を送ります。どちらも作業タブのボタンと中身は同じで、実績を受けたGAS側がスプレッドシートの「作業」と「防除記録」の両方に書きます。"), secHead("４　古い方式(通常は使いません)"), /*#__PURE__*/React.createElement("button", {
  ...
  /*#__PURE__*/React.createElement("button", {
    ...
    style: S.note
  }, "この日以降の散布実績だけを農薬の使用回数として数えます。作期が変わったらこの日付を更新すると、カウントが0からやり直しになります(過去の記録は消えません)。使用回数の上限は薬剤ごとに「🧮薬剤登録・希釈計算」タブの「🧪 薬剤・プリセット」で登録できます。登録していない薬剤は既定の", CHEM_LIMIT_DEFAULT, "回で警告します。"))), /*#__PURE__*/React.createElement("section", {
    style: S.card
    ...
    title: "📡 送信とバックアップ",
    desc: "作業の送信は2つに分かれています。作業前は「🌾 本日の作業圃場登録」の中の「☁ 作業予定を共有」で、登録した圃場(予定)を他の端末へ配ります。散布後は地図/一覧の下の「☁ 実績を送信」で、済ませた圃場の実績を送ります。どちらも「作業日で選んでいる日」に限らず、たまっている未送信ぶんをまとめて送ります。台帳(防除記録)には実績の送信を受けたGAS側が直接書き込みます(予定・調合だけの圃場は「作業」シートには載りますが台帳には載りません)。送信は手動です(v9.38以降、自動では送りません)。圏外でも記録は端末に残るので、電波が戻ってからボタンを押してください。設定タブの「📋 作業予定を送り直す」「🚦 実績を送り直す」は同じ送信のやり直し用です。圃場・薬剤は登録・編集・削除した時点で別に自動送信され、設定タブの「🔁 今すぐ同期する」で送り直せます。端末どうしの共有はチームコードで行い、変わったものだけが自動でやりとりされます(設定タブの「☁↑ 端末→共有へ保存」「☁↓ 共有→端末へ読込」は丸ごと上書きする古い方式で、④に畳んであります)。"
    ...
    notes: [
      ...
      "🧪 引き継ぐのは圃場だけです。薬剤と投下量は空で始まります。希釈倍率はその日の投下量で決まる作りなので、前日の値をそのまま使うと用量を外します。入れ直しは「↩ 前回と同じ薬液」で1押しです",
      "📗 前の日の記録はそのまま残ります。引き継いでも前日の地図は前日のままです",
      "⚠ 同じ日に同じ圃場を2件入れて片方だけ済ませた場合、その圃場は引き継ぎの対象になりません(一度は撒いた圃場と見なすため)。必要なら手で足してください",
      "※ スプレッドシート側(Code.gs)の差し替えは不要です。日付の範囲で取る仕組みは元から入っていました"
    ]
  }, {
    ver: "v8.94",
    date: "2026-08",
    notes: [
      "📍 現在地が出ていないとき、進捗地図に「📍 位置情報を使う」が出るようになりました。押すと位置情報の確認が出ます。許可すればその場で現在地が出ます",
      "🐞 一度「許可しない」を選ぶと、端末の設定で許可し直してもアプリを開き直すまで現在地が出ませんでした。断られた時点で測位を止めたきり、始め直していなかったためです。許可された時点で始め直します",
      "⚠ すでに拒否している場合、ブラウザは二度と確認を出しません。その場合は「⚠ 位置情報が拒否されています」と出て、端末の設定から戻す案内をします",
      "📍 全画面の📍も、現在地が出ていないときは許可を求める側に回ります。押しても何も起きない状態をなくすためです",
      "※ 現在地が出ているときは何も出しません(青い丸が出ていることが答えなので)"
    ]
  }, {
    ver: "v8.93",
    date: "2026-08",
    notes: [
      "📱 全画面のボタンを、地図の右わきに縦一列で置きました。絵文字だけの小さなボタン4つです(✕ 全画面をやめる ／ 📍 現在地 ／ ⊙ 今日の圃場 ／ 🏷 札の出し入れ)",
      "🔍 原因が分かりました。iPhoneでは見出しとタブバーが全画面の覆いより手前に描かれるため、画面の上下に置いたボタンはその裏に回ります。上端に置いたv8.89も、下端に置いたv8.90〜8.92も、これで見えなくなっていました",
      "📐 その状況を作って確かめました。上端・下端に置いたものはどれも押せず(手前にヘッダーやタブバーのボタンがいる)、右わきに置いたものは4つとも押せます。393x852(iPhone15相当)・360x780・852x393(横向き)・1280x720で確認しています",
      "※ ボタンが小さいので、長押しすると名前が出ます"
    ]
  }, {
    ver: "v8.92",
    date: "2026-08",
    notes: [
      "📱 iPhone15の全画面でボタンが出ない件に手を入れました。画面に貼り付ける置き方(position:fixed)をやめ、地図の下に場所を取って並べる形にしました。場所を実際に占めるので、画面外や何かの裏へ押し出されようがありません",
      "⚠ この症状は手元で再現できていません。原因を突き止めたのではなく、原因になりうる仕組みそのものをやめた形です。直っているかは実機で確かめてください",
      "📐 393x852(iPhone15相当)・360x780・768x1024・852x393(横向き)で、4つとも画面内に出ることを確かめています"
    ]
  }, {
    ver: "v8.91",
    date: "2026-08",
    ...
    notes: [
      ...
      "👆 ボタンの高さは45pxあり、指で押す的として十分な大きさを確保しています",
      "※ 「⊙ 今日の圃場へ」は幅の都合で「⊙ 今日の圃場」になりました。働きは同じです"
    ]
  }, {
    ver: "v8.89",
    date: "2026-08",
    notes: [
      "🏷 進捗地図に「🏷 札あり / 🏷 札なし」を付けました。圃場名と面積の札を消せます。全画面でも右上から切り替えられます",
      "🐢 札は見た目ほど安くありません。進捗地図は45秒ごとに全部描き直しますが、PCでの実測で200圃場のとき、札なし 3.5ms に対し札あり 173.7ms かかります(50圃場で 1.4ms→28.2ms、400圃場で 7.6ms→537.5ms)。圃場数に対して加速度的に増え、スマホはこれより数倍遅くなります",
      "※ 札を消しても、圃場をタップすれば名前・面積・実績はこれまでどおり吹き出しに出ます",
      "🔧 選んだ状態は端末に残ります(既定は札あり)"
    ]
  }, {
    ver: "v8.88",
    date: "2026-08",
    ...
    notes: [
      ...
      "📡 送信の説明を実際の動きに合わせました。台帳(防除記録)への送信はその日ぶんだけ・手動ですが、ガイドは「全データを送信」「電波が戻ると自動で再送」と書いていました",
      "※ 文言だけの変更です。計算・送受信・データは変わっていません"
    ]
  }, {
    ver: "v8.81",
    date: "2026-08",
    ...
    isNew: true,
    notes: [
      "🚗 進捗地図で圃場をタップしたときの吹き出しに「🚗 この圃場へナビ」を追加しました。現場は「向かう → 撒く → 記録」の順なので先頭に置いてあります",
      "※ 作業リストの「🚗 ナビ」と同じで、Googleマップのナビを現在地から目的地へ開きます。地図で囲んでいない圃場はボタンが薄くなり、登録方法を案内します"
    ]
  }, {
    ver: "v8.78",
    date: "2026-08",
    ...
    isNew: true,
    notes: [
      "🐞 スプレッドシートの行を探す鍵を「チーム＋ID」にしました。IDだけで探していたため、チームの違う行を上書きしてしまうことがありました",
      "⚠ 具体的には(1)端末のチームコードを変えたあとに作業を触ると、前のチームの行が新しいチームへ移ってしまう (2)別のチームが同じ日に同じ圃場を入れると互いの行を消し合う、の2つです。(2)は v8.73 で作業IDを「日付＋圃場ID」から決めたことで起きるようになったものです",
      "※ スプレッドシート側の Code.gs を差し替えて再デプロイしてください。アプリ側だけ更新しても直りません",
      "🧪 チームが違えば同じIDでも別の行になることを確かめるテストを追加しました(GASテスト 83件 → 88件)"
    ]
  }, {
    ver: "v8.76",
    date: "2026-08",
...
const PROGRESS_STATES = {
  ...
  // 今日の作業には入っていないが、直近の日に散布し終えている圃場。
  // 170圃場を数日かけて回るとき、赤(今日やる)と見分けが付かないと
  // 済んだ場所へまた向かうことになる。
  // 青にしたのは、衛星写真が緑と茶で埋まっているため。緑(実施済)と
  // 同系色にすると、少し離れただけで見分けられない。
  donePrev: {
    fill: "#2F6FA8",
    stroke: "#17415F",
    mark: "✓",
...
const PROGRESS_ORDER = ["done", "planned", "donePrev", "none"];

// ── 何日ぶんさかのぼって「済」を見るか ──
// 選んでいる作業日を含めた日数。3 なら「今日・前日・前々日」。
// 長くするほど古い実績まで青で残るので、次の作期に入ると邪魔になる。
//
// 広げたときの費用を実測した(170圃場・1日あたり170件・作業シート510行)。
//
//   サーバーが読むセル数   1日 12,264 → 3日 12,264(同じ)
//   応答(生)               26.5KB → 79.2KB
//   応答(gzip)             1.2KB → 2.7KB
//   端末で畳む時間         0.26ms → 0.50ms
//   署名と差分             0.22ms → 0.23ms
//
// 読む量が変わらないのは progressItems_ が範囲に関係なくシート全行を
// 読んでから絞るため。増えるのは応答の大きさだけで、それも圧縮が効く。
// 0.3Mbps でも 0.07 秒。45秒ごとの通信量は gzip で 1時間あたり約0.2MB。
//
// ※ gzip が実際に効くかは、Googleのフロントが Accept-Encoding を
...
// 圃場名・面積の札を出すか。
//
// 札は圃場1枚につきDOMを1つ(Leaflet)ないし2つ(Google:名前と面積で別マーカー)
// 作る。効くのはパンより「作り直し」で、札は中央寄せのために実寸を測らせるので
// レイアウトが同期的に走る。進捗地図は45秒ごとに全部作り直すため、そのたびに
// 画面が止まる。
//
// Leaflet 1.9.4 / PC(Chromium・1280x720)での実測。中央値。
//   圃場数   作り直し(札なし → 札あり)   1回のパン(札なし → 札あり)
//     50       1.4ms →  28.2ms            0.51ms → 0.70ms
//    100       2.3ms →  74.5ms            0.48ms → 1.04ms
//    200       3.5ms → 173.7ms            0.64ms → 1.60ms
//    400       7.6ms → 537.5ms            1.32ms → 1.74ms
// 圃場数に対して加速度的に増える。スマホはこれより数倍遅い。
// 計測に使ったページは残していない(tools/ には入れていない)。
//
...
function ProgressLeafletCanvas(p) {
  ...
  React.useEffect(() => {
    if (!ready) return;
    // 一度断られると geoWatch は止まる。許可し直したときに測位を始め直せる
    // よう、親が geoAttempt を1つ増やしてここをやり直させる(v8.94)。
    // これが無いと、設定で許可してもアプリを開き直すまで現在地が出ない。
    geoStopRef.current = geoWatch(navigator, pos => {
      drawGps(pos);
      // 位置が取れたことを親に伝える。案内を引っ込める判断に使う
      p.onGeoFix && p.onGeoFix();
    }, msg => p.flash && p.flash(msg));
    return () => {
...
function ProgressGoogleCanvas(p) {
  ...
  React.useEffect(() => {
    if (!ready) return;
    // Leaflet版と同じ。断られたあと許可し直したときにやり直せるようにする
    geoStopRef.current = geoWatch(navigator, pos => {
      drawGps(pos);
      p.onGeoFix && p.onGeoFix();
    }, msg => p.flash && p.flash(msg));
...
function ProgressMapTab(p) {
  ...
  // ── 位置情報の状態 ──
  // これまで、現在地が出ないときに画面へ何も出していなかった。
  // 断られたのか・測っている最中なのか・端末が非対応なのかが分からず、
  // 直しようがない。状態を出して、そこから許可を出せるようにする(v8.94)。
  const [geoState, setGeoState] = React.useState("unknown");
  const [geoFix, setGeoFix] = React.useState(false);   // 一度でも位置が取れたか
  const [geoAttempt, setGeoAttempt] = React.useState(0); // 増やすと測り直す
  React.useEffect(() => {
    if (!navigator.geolocation) {
      setGeoState("unsupported");
      return;
    }
    // Permissions API は返さない環境もある。そのときは "unknown" のまま。
    // 分からないなら案内を出す側に倒す(geoHintFor)
    ...
    perms.query({
      ...
      st = x;
      setGeoState(x.state);
      // 端末の設定で許可し直したとき、アプリを開き直さずに気づける
      x.onchange = () => {
        if (!alive) return;
        setGeoState(x.state);
        if (x.state === "granted") setGeoAttempt(n => n + 1);
      };
    }).catch(() => {});
    return () => {
      alive = false;
  ...
  // 案内を押したとき。ここで測ると、まだ聞かれていない端末では
  // ブラウザの確認が出る。押したのが操作の直後であることが大事で、
  // 読み込みのついでに測ると確認を出さない端末がある。
  const askGeo = () => {
    if (!navigator.geolocation) {
      setGeoState("unsupported");
      p.flash && p.flash("この端末は位置情報に対応していません");
    ...
    if (geoState === "denied") {
      // 一度断ると、ブラウザは二度と確認を出さない。設定から戻してもらう
      p.flash && p.flash("端末の設定から位置情報を許可してください(iPhoneは 設定 → プライバシーとセキュリティ → 位置情報サービス、またはブラウザの「あ」メニュー →『Webサイトの設定』)");
      return;
    }
    navigator.geolocation.getCurrentPosition(() => {
      setGeoState("granted");
      // 測位を始め直す。断られたあとは watch が止まったままのため
      setGeoAttempt(n => n + 1);
    }, err => {
      if (err && err.code === 1) {
        setGeoState("denied");
        p.flash && p.flash("位置情報が許可されませんでした。端末の設定から許可してください");
      } else {
        p.flash && p.flash("位置情報を取得できませんでした(空の見えない場所では時間がかかります)");
      }
    }, {
      enableHighAccuracy: true,
      timeout: 15000
  ...
  const geoHint = geoHintFor(geoState, geoFix);
  const [showLabels, setShowLabelsState] = React.useState(() => localStorage.getItem("tankmix:proglabels") !== "0");
  const setShowLabels = v => {
    setShowLabelsState(v);
    try {
      localStorage.setItem("tankmix:proglabels", v ? "1" : "0");
    } catch (e) {
  ...
  const refresh = async () => {
    ...
    if (!r || r.error) {
      const e = r && r.error;
      setErr(e === "共有オフ" ? "共有がオフです。画面右上の「🚫 共有オフ」を押すと、最新を取りに行けます" : e === "設定" ? "設定タブで「送信先URL」と「チームコード」を入れてください" : e === "通信" ? "取得できません。電波の届く場所でもう一度お試しください" : e === "GAS" || e === "invalid payload" ? "スプレッドシート側のスクリプトが古い版です。Code.gs を貼り直して「新バージョン」でデプロイしてください" :e === "auth" ? "共有パスワードが違います。設定タブで確認してください" : "取得できません(" + (e || "不明") + ")");
      return;
    }
    const next = {
      items: r.items || [],
  ...
  // ── 開いている間は自動で取り直す ──
  // 他の端末が「☁ 実績を送信」を押すと、その実績がサーバーへ送られる。
  // (v9.38以降、散布済にした瞬間の自動送信はやめ、送信はボタンを押したときだけ)。
  // こちらが取りに行かないと色が変わらないので、進捗地図を出している間だけ
  // 一定間隔で取り直す。
  ...
  const counts = React.useMemo(() => {
    ...
    statusByField.forEach((v, key) => {
      ...
      // 前日までに済(青)はその日の作業ではない。合計に混ぜると
      // 「この日の圃場数」が実際より多く出る
      if (v.status === "donePrev") return;
      c.total++;
  ...
  /*#__PURE__*/React.createElement("div", {
    ...
    areaUnitKey: p.areaUnitKey,
    showLabels,
    // 増えると測位をやり直す。許可し直したあとに現在地を出すため
    geoAttempt,
    onGeoFix: () => setGeoFix(true),
    style: fullMap ? {
      ...S.mapBox,
      borderRadius: 0,
      border: "none"
    } : S.mapBox
    }), fullMap && /*#__PURE__*/React.createElement("div", {
      // ── 全画面の操作は「地図の右わき」に置く ──
      //
      // 上端(v8.89)も下端(v8.90〜8.92)も駄目だった。実機(iPhone15)では
      // 見出しとタブバーが覆い(mapWrapFull)より手前に描かれるため、
      // 画面の上下に置いたものはその裏に回って見えない。
      // 手元のエミュレータでは覆いが最前面に出るので再現できなかった。
      //
      // 上下を使わなければこの問題に当たらない。地図の右わきに縦一列で
      // 置く。position:absolute で覆いを基準にするので、覆いがどこに
      // どう置かれても地図の右わきから動かない(fixed のように画面を
      // 基準にしない)。
      //
      // 絵文字だけにして小さくする。文字を入れると幅が要り、狭い端末で
      // 地図を削る。何のボタンかは title と、押したときの動きで分かる。
      style: S.mapSideBtns,
      className: "no-print"
    }, /*#__PURE__*/React.createElement("button", {
      ...
      onClick: () => {
        ...
        apiRef.current && apiRef.current.locate && apiRef.current.locate();
      },
      style: {
        ...S.mapSideBtn,
        ...(geoHint ? S.mapSideBtnWarn : {})
      },
    ...
    className: "num"
  }, "この日の作業 " + sel.st.total + "件（" + sel.st.doneCount + "件済）"), sel.st && sel.st.status === "donePrev" && /*#__PURE__*/React.createElement("div", {
    // 前の日に済ませた圃場。今日の実績は無いので、数量や記録者は出さない。
    // 「いつ済んだか」だけが要る情報
    style: {
      ...S.smallLabel,
      marginTop: 6,
...
const S = {
  ...
  // ── 進捗地図を全画面にしたときの操作ボタン(v8.93) ──
  //
  // 地図の右わきに縦一列。上端でも下端でもない場所に置く。
  // 実機(iPhone15)では見出しとタブバーが覆い(mapWrapFull)より手前に
  // 描かれるため、画面の上下に置いたものは裏に回って見えなくなる。
  // 上端に置いた v8.89、下端に置いた v8.90〜8.92 が両方これで駄目になった。
  // 手元のエミュレータでは覆いが最前面に出るので再現できない。
  //
  // position:absolute で覆いを基準にする。覆いは position:fixed なので
  // 絶対配置の基準になる。fixed のように画面を基準にしないので、
  // 覆いがどこにどう置かれても地図の右わきから動かない。
  //
  // 縦の位置は真ん中。上下の端から離すほど、見出し・タブバー・ノッチ・
  // ホームバーのどれにも当たらない。
  mapSideBtns: {
    position: "absolute",
    right: 10,
    ...
    // 地図(mapBox)は zIndex 0 の入れ物なので、その上に出す
    zIndex: 2,
    display: "flex",
    flexDirection: "column",
    gap: 8
  ...
  mapSideBtn: {
    // 44px角は指で押す的の下限。文字を入れないので幅もこれで足りる
    width: 44,
    height: 44,
    padding: 0,
    display: "flex",
  ...
  // 位置情報が出ていないときの案内。目立たせるが、赤にはしない。
  // 壊れているのではなく、押せば進む状態なので
  geoHintBtn: {
    background: "#FDF3E0",
    color: "#A15E08",
    border: "1.5px solid #EBD3A8"
  },
  mapSideBtnWarn: {
    background: "rgba(253,243,224,0.96)",
    color: "#A15E08",
    border: "1.5px solid #EBD3A8"
  },
  // OFFのときは沈んで見えるようにする。押せることは変えない
  mapSideBtnOff: {
    background: "rgba(240,243,238,0.94)",
    color: "#8a988e"
  },
  mapSegWrap: {
    display: "flex",

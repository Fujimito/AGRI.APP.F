...
def build_story():
    ...
    s.append(Spacer(1, 2 * mm))
    s.append(note("固形剤は「g」、液剤は「mL」で出ます", [
        "水和剤・顆粒水和剤（DF）・水溶剤・顆粒水溶剤・粒剤・粉剤（DL）・豆つぶ／ジャンボ剤・"
        "ペースト剤は<b>「g」</b>、それ以外は<b>「mL」</b>で表示します。剤型の選択で切り替わります。",
        "アプリは<b>1 mL = 1 g として計算</b>します（比重は見ていません）。"
        "アグリノートへの転記では同じ量を「kg」に直して出します。",
        "<b>粉やつぶを体積で量らないでください。</b>「100 g」と出たら、はかりで100グラムです。",
    ], "amber"))
    ...
    s.append(LI("圃場をタップしたときの「<b>🚗 この圃場へナビ</b>」で、現在地からその圃場までの"
                "案内をGoogleマップで開けます（地図で囲んでいない圃場はボタンが薄くなります）"))
    s.append(Spacer(1, 2 * mm))
    s.append(note("「この端末にない作業が n件、サーバーに残っています」と出たら", [
        "端末から外した作業が、通信の失敗などでサーバー側に残ってしまった状態です。"
        "そのままだと<b>地図が赤いまま戻りません</b>。",
        "「<b>🧹 サーバーから外す</b>」を押すと片付きます。押す前に確認画面が出て、"
        "<b>誰が登録したもの（記録者）か</b>が表示されます。",
        "<b>別の端末が登録した予定なら、その端末からも消えます。</b>"
        "身に覚えのない日付でも、相方が組んだ予定のことがあります。記録者を見てから押してください。",
    ], "amber"))
    ...
    s.append(grid(
        ...
        [42, 132]))
    s.append(Spacer(1, 3 * mm))
    s.append(note("チームコードを変えるとき", [
        "<b>前のチームで登録した圃場・作業・薬剤は、この端末に残ったままです。</b>"
        "勝手には消しません（消すかどうかは使う人が決めることだからです）。",
        "残っているだけなら害はありませんが、<b>そのまま編集すると新しいチームへ送られます</b>。"
        "持ち込みたくなければ、共有をオフにするか、対象の圃場を先に外してください。",
        "変えたあとは設定タブのチームコード欄に注意書きが出ます。",
        "チームコードは<b>大文字と小文字を区別します</b>。jupiter と Jupiter は別のチームです。",
    ], "amber"))
    s.append(Spacer(1, 3 * mm))
    ...
    s.append(grid(
        ...
        [["他の端末に圃場が出ない",
         ...
         ["進捗地図に出ない圃場がある",
          ...
          "「圃場登録」タブで囲んでください。"],
         ["進捗地図の赤（未実施）が消えない",
          "①端末の作業リストが0件なのに赤いなら、サーバーに行が残っています。"
          "地図の下に「この端末にない作業が n件…」と出るので「<b>🧹 サーバーから外す</b>」を押してください。"
          "②その表示が出ないときは、スプレッドシート側の <b>Code.gs が古い版</b>です。貼り替えて再デプロイしてください。"],
         ["送信が急に全部失敗するようになった",
          "スプレッドシートの行数の上限（新規作成時1000行）に達している可能性があります。"
          "<b>Code.gs を最新に貼り替えて再デプロイ</b>してください。以後は自動で行を足し、"
          "削除済みの行は30日で片付けます。"],
         ["同じ圃場を１日に２回散布するとき",
          "午前・午後で分けるなど、同じ圃場をその日の作業に2件入れられます。"
          "<b>進捗地図は「2件とも済むまで赤」</b>です（v8.83で修正）。"
          "地図でその圃場をタップすると「この日の作業 2件（1件済）」と出るので、"
          "残りが何件か分かります。「✓ 散布済にする」はまだ済んでいないほうに入ります。"
          "<b>v8.82以前は、片方を済ませた時点で地図が緑になっていました。</b>"],
         ["「端末の保存領域がいっぱいです」と赤い帯が出た",
          "<b>その帯が出ている間、入力した内容は保存されていません。</b>"
          "画面には出ていますが、アプリを開き直すと消えます。"
          "先に「設定」タブの「データ管理」で書き出して保管し、"
          "そのあと古い日の作業を減らしてください。"
          "帯は「閉じる」を押すまで出たままになります（v8.84から。"
          "それ以前は何も出ず、消えてから気づく状態でした）。"],
         ["圏外で地図が真っ白",
